...
const ContactForm = () => {
  return (
    <form action="/send" method="POST" className="glass-card mt-[3%] pb-[5%] px-[5%] w-[85%] mx-auto">
        ...
        <fieldset className="w-[30%]">
          ...
          {formData.map((item, index) => (
            <div key={index} className="flex flex-col flex-1 gap-1">
              <label htmlFor={item.label.htmlFor} className="mt-[5%]">{item.label.text}</label>
              <input
                id={item.input.id}
      ...
      <fieldset className="w-[55%] flex flex-col h-full">
        <legend className="subtitle card-title w-full mb-2">Your Request or Question</legend>
        
        <label htmlFor="message" className="mt-4 mb-2 text-white/80 text-sm font-medium">Message:</label>
